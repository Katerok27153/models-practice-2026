# ## Программный код для модели Лотки–Вольтерры и графики
...
function compute_fft(signal, dt)
    n = length(signal)
    signal_clean = replace(signal, NaN => 0.0, Inf => 0.0, -Inf => 0.0)
    signal_centered = signal_clean .- mean(signal_clean)
    spectrum = abs.(rfft(signal_centered))
    freq = rfftfreq(n, 1/dt)
    positive_idx = (freq .> 0) .& (spectrum .> 0)
    return freq[positive_idx], spectrum[positive_idx]
end
...
plt5 = plot(freq_prey, spectrum_prey,
    label=[L"Жертвы (x)"],
    xlabel="Частота",
    ylabel="Амплитуда",
...
# Добавляем спектр хищников, если данные корректны
if !isempty(freq_predator) && !isempty(spectrum_predator)
    plot!(plt5, freq_predator, spectrum_predator,
        label=[L"Хищники (y)"],
        color=:red,
        linewidth=1.5)
end
...
if !isempty(spectrum_prey) && length(spectrum_prey) > 1 # Пропускаем нулевую частоту
    idx_prey = argmax(spectrum_prey[2:end]) + 1
    if idx_prey <= length(freq_prey)
...
# Для спектров в панели используем те же данные, но с линейной шкалой, чтобы избежать предупреждений
if !isempty(freq_prey) && !isempty(spectrum_prey)
    plot!(plt6[5], freq_prey, spectrum_prey, label=L"x", color=:green, linewidth=1.5,
        title="Спектр жертв", grid=true)
end
...
# Исправление 6: Упрощенный анализ колебаний с защитой от ошибок
