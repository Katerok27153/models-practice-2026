# ## Анимация процесса (сеть с арбитром)
...
gif(anim, plotsdir("philosophers_simulation_arbiter.gif"), fps = 2)
println("Анимация сохранена в plots/philosophers_simulation_arbiter.gif")
...
p = bar(
    ...
    xlabel = "Позиция",
    ylabel = "Фишки",
    title = "Конечное состояние сети с арбитром (t = $(round(last_row.time, digits=2)))",
)
xticks!(1:length(u_last), string.(names), rotation = 45)
savefig(plotsdir("philosophers_simulation_arbiter_frame.png"))
println("Кадр сохранён в plots/philosophers_simulation_arbiter_frame.png")
